...
if (
  queryParams.effectiveConnectionType !== "" &&
  !ectValues.includes(queryParams.effectiveConnectionType)
) {
  console.log(
    `parameter effectiveConnectionType should be one of allowed values:  ${ectValues.join(
      ","
...
if (!formFactorValues.includes(queryParams.formFactor)) {
  console.log(
    `parameter formFactor should be one of allowed values:  ${formFactorValues.join(
      ","
    )}`
  );
  process.exit();
...
if (!argv.key) {
  console.log(
    `Provide an API key to make CrUX Rest API request \n '--key=[YOUR_API_KEY]'}`
  );
  process.exit();
}
